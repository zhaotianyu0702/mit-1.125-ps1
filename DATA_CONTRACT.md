...
The interface has two main views: `#landscape` for the aggregate opportunity landscape and `#skills` for local skill-mention exploration. `#postings` is the full job-posting list, showing posting experience, skills, direction, location, and source. It does not rank applicants or provide an application channel. Methodology, reflection, presentation, and other assignment materials remain linked from the compact accessible Project files footer.

Skill frequencies count each canonical skill once per posting and include required, preferred and contextual mentions. The heatmap denominator is the number of filtered postings in each direction. For each posting, count canonical selected skills present and divide by its distinct tracked skills. The posting meets the target when matched >= ceil(skill_count * target/100). The site target is fixed at 70%; there is no selector or URL override. Explicit thresholds in the pure analytics functions support offline sensitivity studies only. Skill-less postings remain unscored and are excluded from Skill lab denominators (overall and per role), but stay in landscape and Job postings totals. A one-skill scenario measures the before/after change in target crossings, not newly eligible jobs. The separate closer count includes only improved postings that remain below target, excluding the newly covered group. Tied recommendation gains use summed 1/ceil(skill_count * target/100) progress only on previously unmet postings; already covered postings add no benefit. Re-rank after every selection or filter change. Recommendations exclude selected skills and prioritize new target crossings, then normalized progress on unmet postings, then distinct company count. A suggested practice project is editorial learning guidance, not a source claim. The automated extraction vocabulary is limited; untracked and missing skills are not evidence of absence. Skill profile data stays browser-only, with optional device-local persistence.
...
The reproducible sensitivity study is `research/skill-threshold-study.md` with full results in JSON; regenerate using `node scripts/analyze-skill-thresholds.mjs`. Seventy percent is a fixed product heuristic, chosen using five role-specific profiles and three-step learning paths, not a calibrated employment criterion.
